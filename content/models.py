...
class Show(models.Model):
    ...
    kinopoisk_rating = models.FloatField(default=0.0,validators=[MinValueValidator(0.0), MaxValueValidator(10)], blank=False,)
    imdb_rating = models.FloatField(default=0.0, validators=[MinValueValidator(0.0), MaxValueValidator(10)], blank=False,)
...
class MovieSource(Source):
    movie = models.ForeignKey('Movie', on_delete=models.CASCADE, related_name="movie_sources")
    def __str__(self):
        return f'{self.url}'
...
class EpisodeSource(Source):
    episode = models.ForeignKey('Episode', on_delete=models.CASCADE, related_name="episode_sources")
...
class Movie(models.Model):
    ...
    slug = models.SlugField(unique=True, max_length=255)
    kinopoisk_rating = models.FloatField(default=0.0, validators=[MinValueValidator(0.0), MaxValueValidator(10)],
                                        blank=False, )
    imdb_rating = models.FloatField(default=0.0, validators=[MinValueValidator(0.0), MaxValueValidator(10)],
                                   blank=False, )
    kinopoisk_code = models.CharField(max_length=25, null=True, blank=True)
    imdb_code = models.CharField(max_length=25, null=True, blank=True)
